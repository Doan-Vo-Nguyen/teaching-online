...
class UserService {
  ...
  public async getAllUsers() {
    ...
    return users.map(({ user_id, username, fullname, gender, dob, email, address, phone, profile_picture, role }) => ({
      user_id, username, fullname, gender, dob, email, address, phone, profile_picture, role
    }));
  }
  ...
  public async getUserById(userId: number) {
    ...
    if (!user) throw new ApiError(404, USER_NOT_EXISTS.error.message, USER_NOT_EXISTS.error.details);
    return user;
  }
  ...
  public async updateUser(id: number, userData: any) {
    ...
    if (!user) throw new ApiError(404, USER_NOT_EXISTS.error.message, USER_NOT_EXISTS.error.details);
    if (userData.password) userData.password = bcrypt.hashSync(userData.password, saltRound);
    else delete userData.password;
    return await this.userRepository.update(id, { ...userData, updated_at: new Date() });
  }
  ...
  public async updateUserRole(userId: number, role: Role) {
    ...
    const user = await this.userRepository.findById(userId);
    if (!user) throw new ApiError(404, USER_NOT_EXISTS.error.message, USER_NOT_EXISTS.error.details);
    return await this.userRepository.update(userId, { role, updated_at: new Date() });
  }
  ...
  public async changePassword(id: number, oldPassword: string, newPassword: string) {
    ...
    const user = await this.userRepository.findById(id);
    if (!user) throw new ApiError(404, USER_NOT_EXISTS.error.message, USER_NOT_EXISTS.error.details);
    if (!bcrypt.compareSync(oldPassword, user.password)) throw new ApiError(400, WRONG_OLD_PASSWORD.error.message, WRONG_OLD_PASSWORD.error.details);
  ...
  private validateField(field: any, error: any) {
    if (!field) throw new ApiError(400, error.error.message, error.error.details);
  }

  private handleExistingUser(existedUser: any, username: string, email: string) {
    if (existedUser.username === username) throw new ApiError(409, USERNAME_EXISTS.error.message);
    if (existedUser.email === email) throw new ApiError(409, EMAIL_EXISTS.error.message);
    throw new ApiError(409, USER_EXISTS.error.message);
  }

  private validateUserForClassJoin(user: any) {
    if (!user) throw new ApiError(404, USER_NOT_EXISTS.error.message, USER_NOT_EXISTS.error.details);
    if (user.role !== Role.STUDENT) throw new ApiError(400, NOT_STUDENT.error.message, NOT_STUDENT.error.details);
  }

  private validateClassData(classData: any) {
    if (!classData) throw new ApiError(404, CLASS_NOT_FOUND.error.message, CLASS_NOT_FOUND.error.details);
  }
  ...
  private async checkExistingEnrollment(userId: number, classId: number) {
    ...
    if (existingEnrollment) throw new ApiError(400, ALREADY_ENROLL.error.message, ALREADY_ENROLL.error.details);
  }

  private async validateClassExists(classData: any) {
    const existingClass = await this.classesRepository.findByClassName(classData.class_name);
