...
export class StudentClasses {
    ...
    @ManyToOne(() => Users, user => user.studentClasses, { onDelete: 'CASCADE' }) // Relation with Users n-1
    @JoinColumn({name: "student_id"}) // Column name in the database
    student: Relation<Users>

    @ManyToOne(() => Classes, cls => cls.studentClasses, { onDelete: 'CASCADE' }) // Relation with Classes n-1
