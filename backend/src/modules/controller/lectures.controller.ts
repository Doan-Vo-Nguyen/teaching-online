...
export class LecturesController extends BaseController {
    ...
    private readonly getAllLectures = async (req: Request, res: Response, next: NextFunction) => {
        try {
    ...
    private readonly getLectureById = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const lectureId = parseInt(req.params.id, 10);
            const lectureData = await this.lecturesService.getLectureById(lectureId);
            return sendResponse(res, true, 200, "Get lecture by id successfully", lectureData);
        } catch (error) {
            next(error);
        }
    }

    private readonly createLecture = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const newLecture = await this.lecturesService.createLecture(req.body);
            return sendResponse(res, true, 200, "Create lecture successfully", newLecture);
        } catch (error) {
            next(error);
        }
    }

    private readonly updateLecture = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const lectureId = parseInt(req.params.id, 10);
            const updatedLecture = await this.lecturesService.updateLecture(lectureId, req.body);
    ...
    private readonly deleteLecture = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const lectureId = parseInt(req.params.id, 10);
