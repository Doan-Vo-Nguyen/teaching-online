...
export class StudentClassesRepository extends BaseRepository<StudentClasses> {
    ...
    async findByUserIdAndClassId(student_id: number, class_id: number): Promise<StudentClasses> {
        return this.repository.findOneBy({ student_id, class_id });
    }

    async enrollClass(student_id: number, class_id: number): Promise<StudentClasses> {
